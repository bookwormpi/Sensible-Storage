package bookwormpi.sensiblestorage.client.gui;

import bookwormpi.sensiblestorage.memory.ClientMemoryManager;
import bookwormpi.sensiblestorage.memory.MemorySlot;
...
public class MemoryConfigScreen extends Screen {
    ...
    public MemoryConfigScreen(World world, BlockPos containerPos, int slotIndex, Screen parentScreen) {
        super(Text.translatable("sensiblestorage.gui.memory_config.title"));
        this.world = world;
    ...
    protected void init() {
        ...
        // Search field
        this.searchField = new TextFieldWidget(this.textRenderer, centerX - 75, centerY - 10, 150, 18, Text.translatable("sensiblestorage.gui.memory_config.search"));
        this.searchField.setChangedListener(this::updateFilteredItems);
        this.searchField.setPlaceholder(Text.translatable("sensiblestorage.gui.memory_config.search_placeholder"));
        this.addSelectableChild(this.searchField);
        
        // Action buttons with improved spacing
        this.addDrawableChild(ButtonWidget.builder(
            Text.translatable("sensiblestorage.gui.memory_config.clear_all"),
            button -> {
                allowedItems.clear();
        ...
        this.addDrawableChild(ButtonWidget.builder(
            Text.translatable("sensiblestorage.gui.memory_config.save"),
            button -> {
                saveMemoryConfiguration();
                this.close();
            }
    ...
    public void render(DrawContext context, int mouseX, int mouseY, float delta) {
        ...
        Text slotInfo = Text.translatable("sensiblestorage.gui.memory_config.slot", slotIndex);
    ...
    private void renderAllowedItems(DrawContext context, int centerX, int centerY, int mouseX, int mouseY) {
        Text label = Text.translatable("sensiblestorage.gui.memory_config.allowed_items");
            ...
            if (mouseX >= x && mouseX < x + 16 && mouseY >= y && mouseY < y + 16) {
                ...
                if (this.client != null) {
                    List<Text> tooltip = List.of(
                        stack.getName(),
                        Text.translatable("sensiblestorage.gui.memory_config.click_to_remove")
                    );
    ...
    private void renderAvailableItems(DrawContext context, int centerX, int centerY, int mouseX, int mouseY) {
        Text label = Text.translatable("sensiblestorage.gui.memory_config.available_items");
        ...
        for (int i = visibleStart; i < visibleEnd; i++) {
            ...
            if (mouseX >= x && mouseX < x + 16 && mouseY >= y && mouseY < y + 16) {
                ...
                if (this.client != null) {
                    ...
                    if (alreadyAllowed) {
                        tooltip.add(Text.translatable("sensiblestorage.gui.memory_config.already_allowed"));
                    } else {
                        tooltip.add(Text.translatable("sensiblestorage.gui.memory_config.click_to_add"));
                    }
                    context.drawTooltip(this.textRenderer, tooltip, mouseX, mouseY);
                }
            }
    ...
    private void renderInstructions(DrawContext context, int centerX, int centerY) {
        List<Text> instructions = List.of(
            Text.translatable("sensiblestorage.gui.memory_config.instruction1"),
            Text.translatable("sensiblestorage.gui.memory_config.instruction2")
        );
